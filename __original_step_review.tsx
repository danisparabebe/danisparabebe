            ...
            if (!data.erro) {
                ...
                if (shipRes.ok) {
                    ...
                    if (Array.isArray(shipData) && shipData.length > 0) {
                        ...
                        setShippingOptions([]);
                        toast.info('Frete n├úo dispon├¡vel para este CEP.');
                    }
                }
            } else {
    ...
    const buildCartItems = () => {
        ...
        const newCartItems = items.map(([id, qty]) => {
            ...
            return {
                ...
                productId: `custom-${id}`,
                name: `${getItemLabel(id)} ÔÇö Personalizado`,
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-4 pb-4">
            ...
            <div className="flex flex-col md:flex-row gap-4 items-stretch">
                ...
                {/* ÔöÇÔöÇÔöÇ LEFT COLUMN: PRODUCT DETAILS ÔöÇÔöÇÔöÇ */}
                <div className="w-full md:w-1/2 flex flex-col bg-white border-2 border-[#1f2937] rounded-xl shadow-[4px_4px_0px_rgba(31,41,55,1)]">
                    ...
                    <div className="p-3 border-b border-black/10 bg-[#faf9f7] rounded-t-xl shrink-0">
                        ...
                        <div className="flex items-center gap-2 mt-1.5 text-[10px]">
                            ...
                            <span className="font-black text-[#1f2937]">{selectedThemeName || 'ÔÇö'}</span>
                    ...
                    <div className="p-3 flex-1 flex flex-col gap-3 overflow-y-auto custom-scrollbar">
                        ...
                        {/* Nome do Beb├¬ ÔÇö FULL WIDTH BAND */}
                        ...
                        <div className="shrink-0 bg-white">
                            <p className="text-[9px] font-bold text-slate uppercase tracking-widest leading-none mb-1">Observa├º├Áes do Cliente</p>
                            {detailObs && (
                                <div className="bg-amber-50 border border-amber-300 p-2 rounded mb-1 flex items-start gap-1.5">
                                    <span className="text-amber-800 text-xs">ÔÜá</span>
                                    <p className="text-[10px] font-bold text-amber-950 uppercase leading-snug">{detailObs}</p>
                                </div>
                            )}
                            <input
                                type="text"
                                placeholder="Clique para adicionar ou alterar observa├º├Áes..."
                                value={observations}
                        ...
                        <div className="flex-1 min-h-[100px] border-t border-black/5 pt-2">
                            <div className="flex justify-between items-center mb-1">
                                <p className="text-[9px] font-bold text-slate uppercase tracking-widest">Pe├ºas Selecionadas</p>
                ...
                {/* ÔöÇÔöÇÔöÇ RIGHT COLUMN: CHECKOUT & CTAs ÔöÇÔöÇÔöÇ */}
                <div className="w-full md:w-1/2 flex flex-col bg-white border-2 border-[#1f2937] rounded-xl shadow-[4px_4px_0px_rgba(31,41,55,1)]">
                    ...
                    <div className="p-3 flex-1 flex flex-col gap-3 justify-between">
                        ...
                        <div className="space-y-2.5 shrink-0">
                            ...
                            {/* CPF */}
                            <div className="w-1/2">
                                <label className="block text-[8px] font-bold text-slate uppercase tracking-widest mb-1">CPF (Obrigat├│rio p/ Nota Fiscal)</label>
                            ...
                            {addressLoaded && (
                                <div className="grid grid-cols-[80px_1fr] gap-2">
                                    <div>
                                        <label className="block text-[8px] font-bold text-slate uppercase tracking-widest mb-1">N┬║</label>
                        ...
                        <div className="bg-[#faf9f7] rounded-lg p-3 border border-black/5 mt-2 shrink-0">
                            {addressLoaded && shippingOptions.length > 0 && (
                                <div className="space-y-1 mt-1 mb-2 border-b border-black/5 pb-2">
                                    <label className="block text-[8px] font-bold text-slate uppercase tracking-widest mb-1">Op├º├Áes de Envio</label>
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                                    ...
                                                    <div className="flex items-center gap-1.5 mb-1.5 w-full">
                                                        ...
                                                        {!showAllShipping && (
                                                            <div className="ml-auto flex items-center gap-1 text-[8px] text-slate font-bold uppercase transition-transform">
                                                                Mais Op├º├Áes <ChevronDown className="w-3 h-3" />
                                                            </div>
                                                        )}
                                                    </div>
                                                    
                                                    <div className="flex items-end justify-between w-full mt-auto">
                                                        <p className="text-[8px] text-slate font-medium uppercase min-w-0 pr-1 truncate">Prazo: {primaryOption.days} dias ├║teis</p>
                                                        <span className={`text-[10px] font-black shrink-0 ${primaryDisplayPrice === 0 ? 'text-green-600' : 'text-[#1f2937]'}`}>
                                                            {primaryDisplayPrice === 0 ? 'GR├üTIS' : formatPrice(primaryDisplayPrice)}
                                                        </span>
                                                    </div>
                                                </button>
                                            )
                                        ...
                                        {showAllShipping && shippingOptions
                                            ...
                                            .map((option) => {
                                                const isStateEligible = isEligibleForFreeShipping(formData.state || '');
                                                // Gr├ítis logic technically only applies to cheapest, but just in case we map it:
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <div className="flex items-end justify-between w-full mt-auto">
                                                            <p className="text-[8px] text-slate font-medium uppercase min-w-0 pr-1 truncate">Prazo: {option.days} dias ├║teis</p>
                                                            <span className={`text-[10px] font-black shrink-0 ${displayPrice === 0 ? 'text-green-600' : 'text-[#1f2937]'}`}>
                                                                {displayPrice === 0 ? 'GR├üTIS' : formatPrice(displayPrice)}
                                                            </span>
                                                        </div>
                                                    </button>
                                                );
                            ...
                            {/* Produ├º├úo Artesanal Aviso */}
                            <div className="bg-amber-50 border border-amber-200 rounded p-2 mb-2 flex items-start gap-1.5">
                                <span className="text-amber-600 text-[10px] leading-none mt-0.5">ÔÜá´©Å</span>
                                <p className="text-[8px] font-bold uppercase text-amber-800 leading-snug tracking-wider">
                                    Aten├º├úo: Por conter pe├ºas exclusivas e artesanais sob encomenda, 
                                    o envio ser├í realizado somente ap├│s o prazo de produ├º├úo de <span className="text-amber-950">at├® 12 dias ├║teis</span>.
                                </p>
                            </div>
                                ...
                                <div className="flex justify-between items-start text-[10px] font-bold uppercase text-slate">
                                    ...
                                    <div className="flex flex-col items-end">
                                        <span className={shippingOption ? 'text-[#1f2937]' : 'text-slate/60'}>
                                            {(() => {
                                                if (!shippingOption) return addressLoaded ? 'Indispon├¡vel' : 'A calcular';
                                                ...
                                                return actualPrice === 0 ? 'GR├üTIS' : formatPrice(actualPrice);
                                            })()}
                                        </span>
                                    </div>
                                </div>
                                    ...
                                    if (!isStateEligible && addressLoaded) {
                                        return (
                                            <div className="text-[8px] text-slate/70 font-bold uppercase mb-1">
                                                Frete gr├ítis dispon├¡vel apenas para {FREE_SHIPPING_REGIONS_LABEL}
                                            </div>
                                        );
                                    }
                                    if (FREE_SHIPPING_THRESHOLD - total > 0 && isStateEligible) {
                                        return (
                                            <div className="text-[8px] text-amber-600 font-bold uppercase mb-1">
                                                Faltam apenas {formatPrice(FREE_SHIPPING_THRESHOLD - total)} para <span className="text-amber-700">Frete Gr├ítis</span>!
                                            </div>
                                        );
                                    }
                                ...
                                <div className="border-t border-black/10 mt-2.5 pt-2.5 flex flex-col gap-2.5">
                                    <div className="bg-sage-green/10 border border-sage-green/30 rounded-lg p-2.5 flex justify-between items-center w-full shadow-sm relative overflow-hidden">
                                        ...
                                        <div className="flex flex-col text-left relative z-10">
                                            <span className="text-[10px] font-black text-sage-green-dark uppercase tracking-widest leading-none">Pre├ºo do Enxoval</span>
